...
class ModelEvaluator:
    ...
    def evaluate(
        ...
        for i, example in enumerate(processed_examples):
            try:
                logger.info(
                    f"Processing example {i+1}/{len(processed_examples)}: {example['id']}"
                )
                ...
                processed_result = self.extractor.process_example(
                    prediction=prediction, ground_truth=example["answer"]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max-tokens", type=int, default=100, help="Maximum tokens for model response"
    )
    parser.add_argument(
    ...
    # initialise WandB
    wandb_name = (
        args.wandb_name or f"{args.model}-{args.dataset.split('.')[0]}"
    )  # example: gpt-4o-train
    wandb.init(
