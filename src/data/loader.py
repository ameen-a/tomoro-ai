...
class ConvFinQALoader:
    ...
    def extract_qa_with_context(self, data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Extracted {len(qa_with_context)} question-answer pairs with context")
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i, example in enumerate(examples[:1]):
            ...
            print(f"pre-text: {example['pre_text'][:2] if example['pre_text'] else '[]'}")
            print(f"post-text: {example['post_text'][:2] if example['post_text'] else '[]'}")
